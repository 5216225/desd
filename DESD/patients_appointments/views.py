        ...
        for slot in slots:
            ...
            if len(overlapping_appointments) <= len(available_staff):
                for app in appointment_validation.items():
                    for appointment_details in app[1]:
                        app_start = datetime.combine(date, appointment_details['appointment'].appointment_start)
                        app_end = datetime.combine(date, appointment_details['appointment'].appointment_end)
                        if app_end > datetime_slot_start and app_start < datetime_slot_end: 
                            if appointment_details['patient'] == patient.patient_id:   
                                if appointment_details['staff_id'] in available_staff:
                                    if appointment_details['appointment'].appointment_status == 'cancelled':
                                        state = 3
                                    else:
                                        state = 2
                                else:
                                ...
                                if day_id == 1 or day_id == 5:
                                    if len(app[1]) == 1:
                                        state = 1
                                        selected_staff = appointment_details['staff_id']
                                    else:
                                        booked_staff = int(appointment_details['staff_id'])
                                        staffs_id = available_staff.copy()
                                        if booked_staff in staffs_id:
                                            staffs_id.remove(booked_staff)
                                        if len(staffs_id) != 0:
                                            selected_staff = staffs_id[0]
                                        state = 0   
                                else:
                                    if len(app[1]) == 2:
                                        state = 1
                ...
                for app in appointment_validation.items():
                    for appointment_details in app[1]:
                        if appointment_details['appointment'].appointment_date == date and appointment_details['appointment'].appointment_start.strftime('%H:%M'):
                            selected_staff = appointment_details['staff_id']
                            break
...
def patient_slot_confirmation(request): #frontend rendering code
    ...
    for id in available_staff_id:
        staff_appointments = Appointments.objects.filter(staff_id=id, appointment_date=datetime.strptime(selected_slot.get('date'), '%Y-%m-%d'), appointment_start=datetime.strptime(selected_slot.get('start_time'), '%H:%M'), appointment_end=datetime.strptime(selected_slot.get('end_time'), '%H:%M'))
        if staff_appointments.exists():
            available_staff_id.remove(id)
        else:
...
def patient_appointment_fail(request):
    ...
    return render(request, 'appointment_failed.html', context)
